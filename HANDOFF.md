...
#### ② 【フロントエンド側】アップロード直後の動画サムネイルの自動空撃ちを抑制（静観）
- **改修内容**:
  - `app.js`（行6971）の動画サムネイルで、直近アップロード時や初回描画時にいきなり `preload="metadata"` で公開URLを自動フェッチさせない。
  - `preload="none"` にするか、手元の動画アイコン（🎬）表示にしておき、ユーザーがクリック/ホバーした時だけ読み込む。
- **効果**:
  - アップロード直後の「数秒間の危険時間帯」にブラウザが無駄撃ち通信を飛ばさなくなり、Filebase が準備を終えるまで「そっと待つ」ことができる。
