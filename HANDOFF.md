...
### 期限切れURLへの先行アクセス時に発生する「S3実体の消し漏れ（迷子化）」問題

- **事象**:
  - 有効期限が切れたファイル（例: `content-cache.pages.dev/9xV1abtJ.mp4`）において、管理画面（フロントエンド）を開く前に外部（ブラウザ、クローラー等）から当該URLへアクセスがあると、リンクは正常に404化されKV台帳からも消去されるが、**Filebase S3 上の実体オブジェクトが削除されずに残存してしまう**。

- **根本原因（メカニズム）**:
  1. **配信WorkerはS3キーを持たない**: 配信Worker（`cividge-kv-worker/delivery.js`）はセキュリティ上S3クレデンシャルを持たないため、期限切れアクセス検知時に `IPFS_KV.delete(key)`（KV台帳削除）は行えても、S3の `DeleteObject` は呼べない。
  2. **メタデータの事前消失**: Workerが先にKVキーを削除してしまうため、`expiresAt`（有効期限）の情報がKVから失われる。
  3. **管理画面の判定漏れ**: その後管理画面（フロントエンド）を開いても、当該ファイルは「期限切れファイル」ではなく「KV台帳未登録のS3孤児ファイル（`expiresAt: null`）」に見えるため、フロントエンドの自動回収ロジック（`cleanupExpiredStorageItems`）をすり抜けてS3実体が放置される。

- **解決案（3つのアプローチ）**:
  - **案1（推奨・最も堅牢）: S3削除用 墓標キュー（Tombstone Queue）の導入**:
    - Worker側で最後のリンクが期限切れになった際、KVに `tombstone_s3_<s3Key>`（または `tombstone_s3` プレフィックスのキー）を書き込む。
    - 管理画面（S3の認証情報を持つ側）が開いた際にこの墓標キューを取得し、S3から実体を安全に `DeleteObject` して墓標を削除する。
    - ※既存の自宅Kubo向け墓標（`tombstone_<cid>`）と同様の設計パターンであり、最も非同期親和性が高い。
  - **案2: Worker側で即時KV削除せず「expired」フラグを残す方式**:
    - Workerは期限切れアクセス時に 404 を返しつつも KVキー自体は残し、`expired: true` メタデータをマークする。
    - 管理画面が開いた際に、S3実体削除とKVキー削除を同時に行わせる。
  - **案3: 管理画面に「孤児S3オブジェクト（Orphan Objects）」の清掃機能を持たせる**:
    - S3に実体があるがKV台帳に全く紐付いていないオブジェクトを「孤児ファイル」として検知し、管理画面上で一括または確認の上でクリーンアップできるようにする。
